...
import { getAllEventsByAdmin } from "../../../redux/actions/event";
import Loader from "../../General/Loader";

const AdminDashboardEvents = () => {
  const dispatch = useDispatch();
  const { adminEventsLoading, adminEvents } = useSelector(
    (state) => state.events
  );

  useEffect(() => {
    dispatch(getAllEventsByAdmin());
  }, [dispatch]);

  const [page, setPage] = useState(1);
  const pageSize = 6;
  const totalPages = adminEvents ? Math.ceil(adminEvents.length / pageSize) : 1;
  const paginatedProducts = adminEvents
    ? adminEvents.slice((page - 1) * pageSize, page * pageSize)
    : [];

  return adminEventsLoading ? (
